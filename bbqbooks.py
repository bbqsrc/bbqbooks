...
class BookTUI:
    ...
    def add_invoice(self):
        ...
        def add_item():
            ...
            for q in questions:
                ...
                if q[1] == "tax" and data['their_locale'] != "" and\
                        data['their_locale'] != data['locale']:
                    item['their_tax'] = self.convert_currency(res, data['locale'], data['their_locale'])

                if q[1] == "rate_price" and data['their_locale'] != "" and\
                        data['their_locale'] != data['locale']:
                    item['their_rate_price'] = self.convert_currency(res, data['locale'], data['their_locale'])
        ...
        questions = (
            ...
            ("Invoicee's locale", "their_locale", False, ""),
    ...
    def output_invoice(self):
        ...
        inv_code = input("> ").strip()
        if not self.book.inv_code_exists(inv_code):
            print("Error: invoice code '%s' does not exist." % inv_code)
            return
    ...
    def convert_currency(self, value, in_locale_code, out_locale_code):
        in_currency = self.book.data['locales'][in_locale_code]['currency']['code']
        out_currency = self.book.data['locales'][out_locale_code]['currency']['code']
        req = urllib.request.urlopen("http://www.google.com/ig/calculator?hl=en&q=%s%s=?%s" %
                (value, in_currency, out_currency))
        return yaml.load(req.read().decode())['rhs'].split(" ", 1)[0]
...
class Book:
    ...
    def merge_data_for_invoice(self, invoice):
        return {
            ...
            'inv_locale': self.data['locales'][invoice.get('their_locale', invoice['locale'])]
        }
